  ...
  React.useEffect(() => {
    async function handleColorExtract() {
      ...
      function compareColors(color1, color2) {
        ...
        let luminance2 = 0.2126 * color2[0] + 0.7152 * color2[1] + 0.0722 * color2[2];
  ...
  return (
    ...
    >
        ...
        <defs>
          <linearGradient id="gradient" x1="0" x2="0" y1="0" y2="1">
            <motion.stop
              ...
              initial={{ stopColor: `rgba(${Math.min(255, color[0] * 1)},${Math.min(255, color[1] * 1)},${Math.min(255, color[2] * 1)},1)` }}
              animate={{ stopColor: `rgba(${Math.min(255, color[0] * 1)},${Math.min(255, color[1] * 1)},${Math.min(255, color[2] * 1)},1)` }}
            />
      ...
      <motion.div
        ...
        transition={{ duration: 0.5 }}
        initial={{
          background: `radial-gradient(300% 100% at 50% 100%, black 20%, rgba(${Math.min(255, color[0] * 1)},${Math.min(255, color[1] * 1)},${Math.min(
            255,
            color[2] * 1
          )},1) 100%)`,
        }}
      ...
      >
        {props.imageurl ? (
          <Box
            component="img"
            sx={{ width: "170px", height: "170px", objectFit: "cover", borderRadius: "10px", marginTop: "30px", boxShadow: "0px 20px 20px 20px rgba(0, 0, 0, 0.35)" }}
            src={props.imageurl}
            alt="img"
          />
        ) : (
          <Skeleton variant="rectangular" sx={{ marginTop: "30px", zIndex: "2" }} width="170px" height="170px" />
        )}
        <Typography component="div" variant="h5" sx={{ fontFamily: inter, fontWeight: "600", letterSpacing: "-1.4px", marginTop: "30px", fontSize: "15px", color: "white" }}>
          {props.header}
        </Typography>
        {props.imageurl && (
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-around", width: "100%", marginTop: "20px" }}>
            <Box sx={{ background: "#1fd860", borderRadius: "100%" }}>
              <IconButton onClick={handlePlayPause} aria-label="play/pause">
                {!isPlaying && <PlayArrowIcon sx={{ height: 20, width: 20, color: "black" }} />}
                {isPlaying && <PauseIcon sx={{ height: 20, width: 20, color: "black" }} />}
              </IconButton>
            </Box>
            <Box sx={{ background: "#1fd860", borderRadius: "100%" }}>
              <Link href={props.link128kpbs} sx={{ WebkitTapHighlightColor: "transparent" }}>
                <IconButton aria-label="download">
                  <DownloadIcon sx={{ height: 20, width: 20, color: "black" }} />
                </IconButton>
              </Link>
            </Box>
          </Box>
